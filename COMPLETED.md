...
* Added `[public_nodes]` group to `ansible/hosts.ini` (lines 23-28) with all 4 public nodes
* Nodes: pam-amd1 (VPN gateway), angela-amd2, stanley-arm1, toby-gcp1
...
* Staged rollout order: toby-gcp1 → stanley-arm1 → pam-amd1 → angela-amd2
...
* Staged rollout: toby-gcp1 → stanley → pam-amd1 → angela
...
### ✅ Standardize Oracle Server Naming Convention

**Completed:** Already implemented  
**Branch:** N/A

* Server naming already standardized in `oracle/servers.tf`: `pam-amd1`, `angela-amd2`, `stanley-arm1`
* All servers use consistent naming across Terraform, Ansible inventory, and k3s labels
* No further action needed - naming convention is consistent across all tools

---

### ✅ Add Docs-Only Lint Job to CI

**Completed:** Already implemented  
**Branch:** N/A

* Consolidated lint job in `.github/workflows/actions.yml` includes markdownlint via pre-commit
* Comment in workflow: "markdownlint included via pre-commit, no separate docs-lint needed"
* Pre-commit hooks run on all PRs and check changed files automatically
* No separate docs-only lint job needed - handled by existing lint job
...
**Infrastructure & Configuration:** 2 Tasks
