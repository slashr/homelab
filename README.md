...
- IP addresses are managed via the `master_node_ip` and `public_dns_ip` variables defined in `ansible/group_vars/all.yml` and `kubernetes/variables.tf`
...
- The `kubernetes/variables.tf` file defines `master_node_ip` and `public_dns_ip` for the K3S master node
